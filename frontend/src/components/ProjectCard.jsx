...
import MilestoneModal from "./MilestoneModal"; // Import the new modal
...
const ProjectCard = ({ project, onRefresh }) => {
  ...
  const [isMilestoneOpen, setIsMilestoneOpen] = useState(false); // State for Milestone Modal
  ...
  return (
    <>
              ...
              {isOwner ? (
                <div className="flex items-center gap-2">
                  <button
                    onClick={(e) => { 
                      e.stopPropagation(); 
                      setIsMilestoneOpen(true); // Open the Milestone Modal
                    }}
      ...
      {/* Render Modal Outside the card for z-index purposes */}
      <MilestoneModal 
